...
// A second behavior claiming an id that is ALREADY active is refused.
//
// The id is the only handle arbitration, composition and eviction have on a
// behavior: Admit resolves ownership by id, composeTick samples contributions
// keyed by id, and evict removes by id. Two live behaviors sharing one means
// arbitration can name an owner and composition sample the OTHER one's
// contribution — a mismatch nothing downstream can detect, since both answers
// are individually well-formed. So it is refused at the door, with the same
// named drop any other refusal gets.
func TestADuplicateBehaviorIDIsRefused(t *testing.T) {
	const period = 20 * time.Millisecond
	h := newHarness(t, period, nil)

	first := Behavior{
		ID: "same-id", Action: "ramp", Class: ClassStoppable, Lifetime: Lifetime{Loops: true},
	}
	second := first
	second.Action = "cycle"
	...
	var admitErr error
	var active []string
	seam := func(c TickContext) {
		if c.Tick != 1 {
			return
		}
		if _, err := c.Admit(first); err != nil {
			t.Errorf("the first admission was refused: %v", err)
		}
		_, admitErr = c.Admit(second)
		active = c.ActiveNames()
	}
	h.eng.Send(SetSeamCmd{Seam: seam})
	h.start()
	h.clock.Advance(3 * period)
	if err := h.stop(); err != nil {
		t.Fatalf("Run: %v", err)
	...
	if admitErr == nil {
		t.Fatal("admitting a duplicate id returned no error")
	}
	if !strings.Contains(admitErr.Error(), "same-id") {
		t.Errorf("the refusal does not name the offending id: %v", admitErr)
	}
	if len(active) != 1 {
		t.Errorf("active behaviors after the refused admission = %v, want just the first",
			active)
	}
	if !h.hasDrop("admit", "duplicate-id") {
		t.Fatalf("no duplicate-id drop line was emitted; the log was:\n%s", h.log.String())
	}
}

// The inbox path refuses it too, and the drop names the same reason rather
// than the generic "refused".
func TestADuplicateIDOverTheInboxIsANamedDrop(t *testing.T) {
	const period = 20 * time.Millisecond
	h := newHarness(t, period, nil)
	b := Behavior{
		ID: "twice", Action: "ramp", Class: ClassStoppable, Lifetime: Lifetime{Loops: true},
	}
	h.eng.Send(AdmitCmd{Behavior: b})
	h.eng.Send(AdmitCmd{Behavior: b})

	var active []string
	h.eng.Send(SetSeamCmd{Seam: func(c TickContext) { active = c.ActiveNames() }})
	h.start()
	h.clock.Advance(3 * period)
	if err := h.stop(); err != nil {
		t.Fatalf("Run: %v", err)
	...
	if len(active) != 1 {
		t.Errorf("active behaviors = %v, want exactly one", active)
	}
	if !h.hasDrop("admit", "duplicate-id") {
		t.Fatalf("no duplicate-id drop line was emitted; the log was:\n%s", h.log.String())
	}
...
// An auto-generated id must be unique BY CONSTRUCTION, including against an
// explicit id a client already chose that happens to match the counter's next
// output. Refusing the auto-generated case would punish a client for a
// collision it did not cause.
func TestAnAutoGeneratedIDNeverCollidesWithAnExplicitOne(t *testing.T) {
	const period = 20 * time.Millisecond
	h := newHarness(t, period, nil)
	...
	seam := func(c TickContext) {
		...
		// "ramp-1" is exactly what nextID would mint for the first ramp.
		if _, err := c.Admit(Behavior{
			ID: "ramp-1", Action: "ramp", Class: ClassStoppable,
			Lifetime: Lifetime{Loops: true},
		}); err != nil {
			t.Errorf("the explicit admission was refused: %v", err)
		}
		if _, err := c.Admit(Behavior{
			Action: "ramp", Class: ClassStoppable, Lifetime: Lifetime{Loops: true},
		}); err != nil {
			t.Errorf("the auto-id admission was refused: %v", err)
		}
		for _, ab := range c.engine.active {
			ids = append(ids, ab.behavior.ID)
		}
	}
	h.eng.Send(SetSeamCmd{Seam: seam})
	h.start()
	...
	if len(ids) != 2 {
		t.Fatalf("active ids = %v, want two behaviors", ids)
	}
	if ids[0] == ids[1] {
		t.Fatalf("the auto-generated id collided with the explicit one: %v", ids)
	}
}
