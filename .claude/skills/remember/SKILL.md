...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope neurosymbolic-system
...
scope:** the default personal scope is **private** (`--scope neurosymbolic-system
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `neurosymbolic-system`). Private records are served only to a recall in the same
