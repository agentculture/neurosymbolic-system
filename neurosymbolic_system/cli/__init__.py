"""Unified CLI entry point for neurosymbolic-system.

The agent-first global verbs (``whoami``, ``learn``, ``explain``, ``overview``,
``doctor``) are registered here under :mod:`neurosymbolic_system.cli._commands`,
alongside the ``cli`` noun group. Future noun groups register via their own
``register()`` functions following the same pattern.

Error propagation contract
--------------------------
Every handler raises :class:`neurosymbolic_system.cli._errors.CliError` on
failure; ``main()`` catches it via :func:`_dispatch` and routes through
:mod:`neurosymbolic_system.cli._output`. Unknown exceptions are wrapped into a
...
from neurosymbolic_system.cli._output import emit_error

_ISSUES_URL = "https://github.com/agentculture/neurosymbolic-system/issues"
